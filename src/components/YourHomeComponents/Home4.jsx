...
function Home4() {
  ...
  const useUplaodData = async (e) => {
    ...
    try {
      ...
      for (let i = 0; i < imgFiles.length; i++) {
        const imageRef = ref(
          storage,
          `${auth.currentUser.email}/${imgFiles[i].name}`
        );
        await uploadBytes(imageRef, imgFiles[i]);
  ...
  return (
    <>
          ...
          >
            ...
            <div className="flex  justify-center rounded py-2">
              <button
                ...
                }  flex gap-2 text-white p-3 rounded bg-[#FE375C] `}
              >
                {!uploading ? (
                  <>
                    <span>Publish your home</span>{" "}
                  </>
                ) : (
                  <>
                    <span>Publishing your home</span> <Spinner />{" "}
                  </>
                )}
              </button>
            </div>
      ...
      <div
        className={`w-full  border-t-2 py-4 bg-white flex  justify-between px-6 sticky   bottom-0`}
      >
        <NavLink to={"/yourhome3"} className="underline">
          back
        </NavLink>
      </div>
    </>
